...
#endif
unichar CSFISO8601DateDefaultTimeSeparatorCharacter = DEFAULT_TIME_SEPARATOR;
...
@interface CSFISO8601DateFormatter()

@property (atomic, strong) NSCalendar *parsingCalendar;
@property (atomic, strong) NSCalendar *unparsingCalendar;
@property (nonatomic, strong) NSString *lastUsedFormatString;
...
@implementation CSFISO8601DateFormatter
...
- (id) init {
    self = [super init];
	if (self) {
		self.parsingCalendar = [self makeCalendarWithDesiredConfiguration];
		self.unparsingCalendar = [self makeCalendarWithDesiredConfiguration];

		self.format = CSFISO8601DateFormatCalendar;
		self.timeSeparator = CSFISO8601DateDefaultTimeSeparatorCharacter;
	}
	return self;
}

- (void)setDefaultTimeZone:(NSTimeZone *)tz {
	if (_defaultTimeZone != tz) {
        _defaultTimeZone = tz;
        
        @synchronized(self.unparsingCalendar) {
            self.unparsingCalendar.timeZone = tz;
        }
	}
}
...
- (NSDateComponents *) dateComponentsFromString:(NSString *)string timeZone:(out NSTimeZone **)outTimeZone range:(out NSRange *)outRange {
    ...
    @synchronized(self.parsingCalendar) {
         nowComponents = [self.parsingCalendar components:(NSCalendarUnitYear | NSCalendarUnitMonth | NSCalendarUnitDay) fromDate:[NSDate date]];
    }
	...
	if (strict) timeSep = CSFISO8601DateDefaultTimeSeparatorCharacter;
...
- (NSString *) stringFromDate:(NSDate *)date timeZone:(NSTimeZone *)timeZone {
	switch (self.format) {
		case CSFISO8601DateFormatCalendar:
			return [self stringFromDate:date formatString:ISO_CALENDAR_DATE_FORMAT timeZone:timeZone];
		case CSFISO8601DateFormatWeek:
			return [self weekDateStringForDate:date timeZone:timeZone];
		case CSFISO8601DateFormatOrdinal:
			return [self stringFromDate:date formatString:ISO_ORDINAL_DATE_FORMAT timeZone:timeZone];
		default:
			[NSException raise:NSInternalInconsistencyException format:@"self.format was %lu, not calendar (%d), week (%d), or ordinal (%d)", (unsigned long)self.format, CSFISO8601DateFormatCalendar, CSFISO8601DateFormatWeek, CSFISO8601DateFormatOrdinal];
			return nil;
	}
}
...
- (NSString *) stringFromDate:(NSDate *)date formatString:(NSString *)dateFormat timeZone:(NSTimeZone *)timeZone {
    ...
    @synchronized(self.unparsingCalendar) {
        self.unparsingCalendar.timeZone = timeZone;
    }
    
	if (dateFormat != self.lastUsedFormatString) {
    ...
    @synchronized(self.unparsingCalendar) {
        self.unparsingCalendar.timeZone = self.defaultTimeZone;
    }
...
- (NSString *) weekDateStringForDate:(NSDate *)date timeZone:(NSTimeZone *)timeZone {
    ...
    @synchronized(self.unparsingCalendar) {
        self.unparsingCalendar.timeZone = timeZone;
        components = [self.unparsingCalendar components:NSCalendarUnitYear | NSCalendarUnitWeekday | NSCalendarUnitDay fromDate:date];
    
        startOfYearComponents = [self.unparsingCalendar components:NSCalendarUnitYear fromDate:date];
        ordinalComponents = [self.unparsingCalendar components:NSCalendarUnitDay fromDate:[self.unparsingCalendar dateFromComponents:startOfYearComponents] toDate:date options:0];
    }
	...
	if(includeTime) {
		...
		unichar timeSep = self.timeSeparator;
		if (!timeSep) timeSep = CSFISO8601DateDefaultTimeSeparatorCharacter;
